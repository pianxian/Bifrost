...
@implementation YOYURIRegisterManager
...
{
    [YOYURIRegisterManager registerUrlNaviForClass:[YOYURINavigationCenter class]];
}
...
+ (void)registerUrlNaviForClass:(Class)clazz
{
    ...
    if (methods && methodCount > 0) {
        YOYURINavigationCenter *shareObj = [YOYURINavigationCenter sharedObject];
