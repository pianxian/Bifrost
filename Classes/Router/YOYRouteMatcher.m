...
static NSString * const YOYRouteParameterPattern = @":[a-zA-Z0-9-_%]+";
static NSString * const PXURLParameterPattern = @"([^/]+)";

@interface YOYRouteMatcher ()
...
@implementation YOYRouteMatcher
...
- (NSRegularExpression *)regex {
    if (!_regex) {
        ...
        NSRegularExpression *parameterRegex = [NSRegularExpression regularExpressionWithPattern:YOYRouteParameterPattern
                                                                                        options:0
                                                                                          error:nil];
...
#ifdef DEBUG
            [YOYRouteMatcher showExceptionAlert];
#endif
            return NO;
        }
    }else {
...
- (BOOL)matchStandardURIWithRoute:(YOYRoute *)route isMatchScheme:(BOOL *)isMatchScheme {
...
- (BOOL)matchOldURIWithRoute:(YOYRoute *)route {
    NSString *routeString = route.destURL.standardizedURL.absoluteString;
    if (route.destURL.query) {
...
+ (void)showExceptionAlert
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        canShow = NO;
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"YOYRouter 发生异常"
                                                                                 message:@""
