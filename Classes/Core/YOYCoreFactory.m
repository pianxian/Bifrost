...
+ (Class)classForProtocol:(Protocol *)protocol
{
    @synchronized (protocolClassMap()) {
        return ([protocolClassMap() objectForKey:NSStringFromProtocol(protocol)]
                ?: [YOYSectionDataMapper.sharedInstance mappedClassForProtocol:protocol]);
    }
}
...
{
    return [YOYSectionDataMapper.sharedInstance mappedClassForProtocol:protocol];
}
...
{
    return [[YOYCoreIMapper sharedObject] miki_matchProtocolWithClass:protocol];
}
...
{
    YOYCoreProtocolGeneratorBlock generator = [protocolGeneratorMap() objectForKey:NSStringFromProtocol(protocol)];
    if (generator) {
